...
def picky_piggy(score):
    ...
    """
    # BEGIN PROBLEM 2
    # remainder due to 1/7's repeating 6-digit pattern
    return 7 if score == 0 or score % 6 == 0 else int(0.142857 * 10 ** (score % 6) % 10)
    ...
    if score == 0:  # manual checker
        return 7
    else:
        n %= 6          # find the remainder because pattern repeats every 6 digits
        if n == 0:
            return 7
        while n > 0:           
            one_seventh *= 10   # moves decimal place 1 to the left
            n -= 1  
        one_seventh %= 10   # removes all unnecessary digits
    return int(one_seventh) # ensures integer not double

    one_seventh_values = [7, 1, 4, 2, 8, 5]
    return 7 if score == 0 else one_seventh_values[score % 6]        

    value = 1/7 * 10 ** 1 % 10 / 10
    142857
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def commentary(score0, score1, who=who, last_score=last_score, highest_gain=running_high):
        if who == 0:
        ...
        if difference > highest_gain:
            print(str(difference) + " point(s)! That's a record gain for Player " + str(who) + "!")
            highest_gain = difference
        return announce_highest(who, last_score, highest_gain)
    return commentary
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_rolls, min_rolls, max_score = 1, 1, 0
    averaged_dice = make_averaged(roll_dice, trials_count)
    
    while num_rolls <= 10:
        score = averaged_dice(num_rolls, dice)
        if score > max_score:
            max_score = score
            min_rolls = num_rolls
        num_rolls += 1
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if hog_pile(score + picky_piggy(opponent_score), opponent_score) > 0 or picky_piggy_strategy(score, opponent_score, cutoff, num_rolls) == 0 else num_rolls
    # END PROBLEM 11
